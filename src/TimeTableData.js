      ...
      <div>
        ...
        <article className="dt w-100 bb b--black-05 pb2 mt2" href="#0">
          ...
          <div className="pa1 lh-copy">
                <Link to='/newTT' className="f4 fw6 db white link hover-red">Add TimeTable </Link>
            </div>
          </div>
          </article>
        ...
        <Query query={SCHOOL_TTS_QUERY} variables={{ email: "silverbull@187.guru" }}>
          {({ data, loading }) => {
            ...
            // console.log(data)
            return data.school.timeTables.edges.map(node => (
              <TimeTable data={node.node} key={node.node.id} />
            ));
          }}
        </Query>
      </div>
